...
public class Main
{
	private static boolean rift = false;
	...
	public static boolean initialize(Constants.EntryPoint entryPoint) {
		...
		rift = ((entryPoint == Constants.EntryPoint.RIFT)
				|| Reflection.doesClassExist("org.dimdev.riftloader.RiftLoader"));
		if (rift) {
			Logger.Log("Rift is installed.");
		} else {
			Logger.Log("Rift is not installed.");
		}
	...
	public static boolean findOnTickMethod(boolean print_always) {
		...
		for (OnTickMethod method : config.onTickMethodOrder) {
			switch (method) {
				...
				case RIFT:
					if (rift) {
						onTickMethod = OnTickMethod.RIFT;
						if (print_always || onTickMethod != previous_method)
							Logger.Log("Using Rift for the mod operation.");
						return true;
					}
					break;
			}
	...
	private static void onUpdateInGui(GuiScreen currentScreen) {
		...
		if (MouseUtil.isMouseButtonDown(GLFW.GLFW_MOUSE_BUTTON_RIGHT)) {
			if (!oldRMBDown)
		...
		if (oldSelectedSlot != selectedSlot) {
			...
			if (MouseUtil.isMouseButtonDown(GLFW.GLFW_MOUSE_BUTTON_RIGHT)) { // Right mouse button
				if (config.rmbTweak) {
			...
			} else if (MouseUtil.isMouseButtonDown(GLFW.GLFW_MOUSE_BUTTON_LEFT)) { // Left mouse button
				if (!stackOnMouse.isEmpty()) {
